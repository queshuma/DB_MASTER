...
// 手动分词
const manualTokenize = async (record) => {
  try {
    loading.value = true;
    // 调用接口获取文件内容，使用GET请求并通过params传递fileName
    const response = await link('/file/getRagFileContent', 'get', {}, { fileName: record.name }, {}, true);
    
    // 获取文件内容
    const fileContent = response.data || '';
    
    // 跳转到手动分词页面，并传递文件名和文件内容
    router.push({
